...
if (Platform.OS === "ios") {
  AppState.addEventListener("change", (e) => {
    if (e === "active") RNFetchBlob.emitExpiredEvent(() => {});
  });
}
...
function wrap(path: string): string {
  const prefix = path.startsWith("content://")
    ? "RNFetchBlob-content://"
    : "RNFetchBlob-file://";
  return prefix + path;
}
 ...
 *         @property {boolean} trustyCn
 *                   Trust only certificates with given common name.
 *                   To be used only with trusty set to true.
...
function fetch(...args: any): Promise {
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    subscriptionUpload = emitter.addListener(
      "RNFetchBlobProgress-upload",
      (e) => {
    ...
    req(
      ...
      (err, rawType, data) => {
        ...
        if (err) reject(new Error(err, respInfo));
        else {
          ...
          if (
            ...
            options.key ||
            (options.auto && respInfo.respType === "blob")
          ) {
...
class FetchBlobResponse {
  ...
  readStream: (encode: "utf8" | "ascii" | "base64") => RNFetchBlobStream | null;
  ...
  constructor(taskId: string, info: RNFetchBlobResponseInfo, data: any) {
      ...
      return new Promise((resolve, reject) => {
        switch (this.type) {
          ...
          case "path":
            ...
            break;
          default:
            polyfill.Blob.build(this.data, { type: "text/plain" }).then(
              resolve
            );
            break;
        }
    ...
    this.text = (): string | Promise<any> => {
      ...
      switch (this.type) {
        ...
        case "path":
          return fs
            .readFile(this.data, "base64")
            .then((b64) => Promise.resolve(base64.decode(b64)));
        default:
          return this.data;
      }
    };
     ...
     */
    this.path = () => {
      if (this.type === "path") return this.data;
      return null;
    };

    this.session = (name: string): RNFetchBlobSession | null => {
      if (this.type === "path") return session(name).add(this.data);
      else {
    ...
    this.readStream = (
      ...
      if (this.type === "path") {
        ...
        console.warn(
          "RNFetchblob",
          "this response data does not contains any available stream"
        );
        return null;
      }
    };
    ...
    this.readFile = (encoding: "base64" | "utf8" | "ascii") => {
      if (this.type === "path") {
        ...
        console.warn(
          "RNFetchblob",
          "this response does not contains a readable file"
        );
        return null;
      }
    };
